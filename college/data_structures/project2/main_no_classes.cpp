...
short readData(char* filename, char codes[], short capacity) //parameters??? PostNet codes[]
{
	ofstream output;
	...
	input.open(filename); //declare open instance, change to project02input.txt
	if (!input)
	{
		cout << "Could not open " << filename << endl; //change to project02input.txt
		return 0;
	}

	char line[1000]; //max size for line. excessive?
	...
	output << endl;
			...
			//output << z << " ";
			//output << "1" << tempBarArray << "1" << endl;
			//cout << "after printed conversion" << endl;
			//cout << endl;
		}
		else
		{
...
int main()
{
	char data[1000];
	short size;
	size = readData("project02input.txt", data, 1000);

	PostNet zipcode;
	//zipcode.readData("project02input.txt", data, 1000);
	zipcode.setZip();
	zipcode.getZip();
	PostNet barcode;
	barcode.setBar();
